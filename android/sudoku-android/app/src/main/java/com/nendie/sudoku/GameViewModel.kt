...
enum class GameMode { DIFF, DAILY }
...
class GameViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        val (name, address) = loadProfilePrefs()
        ...
        state = state.copy(profileName = name, profileAddress = address, errorMode = errorMode)
        refreshHomeData()
    }
    ...
    fun startNewGame(difficulty: Difficulty) {
        state = state.copy(mode = GameMode.DIFF, difficulty = difficulty)
        startGeneration(seed = null)
    }

    fun startDaily() {
        state = state.copy(mode = GameMode.DAILY, difficulty = Difficulty.NORMAL)
    ...
    private fun startGeneration(seed: Long?) {
        ...
        generationJob = viewModelScope.launch {
            val puzzle = withContext(Dispatchers.Default) {
                if (seed != null) SudokuEngine.generate(state.difficulty, seed)
                else SudokuEngine.generate(state.difficulty)
            }
            val givens = buildSet {
    ...
    fun startRanking() {
        val (name, address) = loadProfilePrefs()
        state = state.copy(screen = Screen.RANKING, profileName = name, profileAddress = address)
        loadRanking("easy")
    }
    ...
    fun saveProfile(name: String, address: String) {
        ...
        val a = address.trim().ifBlank { "本地" }
        state = state.copy(profileName = n, profileAddress = a)
        prefs.edit().putString(KEY_PROFILE, "$n|$a").apply()
    ...
    private fun loadRanking(mode: String) {
        ...
        val (name, address) = loadProfilePrefs()
        val mine = entries.filter { it.name == name && it.address == address }
    ...
    private fun loadRankingEntries(mode: String): List<RankingEntry> {
        ...
        return raw.lineSequence().mapNotNull { line ->
            ...
            if (p.size < 5) null
                ...
                name = p[0],
                address = p[1],
                elapsed = p[2].toIntOrNull() ?: 0,
                mistakes = p[3].toIntOrNull() ?: 0,
                ts = p[4].toLongOrNull() ?: 0L
            )
        }.toList()
    }
    ...
    private fun recordScore() {
        val (name, address) = loadProfilePrefs()
        val mode = if (state.mode == GameMode.DAILY) "daily" else state.difficulty.name.lowercase()
        val entries = loadRankingEntries(mode).toMutableList()
        entries += RankingEntry(name, address, state.elapsedSeconds, state.mistakes, System.currentTimeMillis())
        val trimmed = if (entries.size > 200) entries.sortedBy { it.ts }.takeLast(200) else entries
        val data = trimmed.joinToString("\n") {
            "${it.name}|${it.address}|${it.elapsed}|${it.mistakes}|${it.ts}"
        }
    ...
    fun hint() {
        ...
        if (s.status != GameStatus.PLAYING) return
        val cells = curCells()
        var target = if (s.selected != -1 && cells[s.selected] == 0) s.selected else -1
        if (target == -1) {
            for (i in 0 until 81) {
                if (cells[i] == 0) {
                    target = i
    ...
    fun restart() {
        if (state.mode == GameMode.DAILY) startDaily() else startNewGame(state.difficulty)
    }
    ...
    private fun parseGame(str: String?): GameUiState? {
        ...
        val puzzle = parseInts(parts[5]) ?: return null
        val solution = parseInts(parts[6]) ?: return null
        val cells = parseInts(parts[7]) ?: return null
        val notes = parseInts(parts[8]) ?: return null
        return GameUiState(
            screen = Screen.GAME,
            status = if (parts[4] == "2") GameStatus.WON else GameStatus.PLAYING,
            ...
            cells = cells,
            notes = notes,
            elapsedSeconds = parts[2].toIntOrNull() ?: 0,
            mistakes = parts[3].toIntOrNull() ?: 0
        )
    }
    ...
    private fun parseRecord(str: String?): SavedRecord? {
        ...
        return SavedRecord(
            mode = enumOr(GameMode.DIFF, parts[0]),
            difficulty = enumOr(Difficulty.NORMAL, parts[1]),
            elapsed = parts[2].toIntOrNull() ?: 0,
            mistakes = parts[3].toIntOrNull() ?: 0,
            completedAt = parts[4].toLongOrNull() ?: 0L,
            puzzle = puzzle,
            solution = solution
        )
    }
    ...
    private fun loadProfilePrefs(): Pair<String, String> {
        val raw = prefs.getString(KEY_PROFILE, null) ?: return "玩家" to "本地"
        val parts = raw.split("|")
        val name = parts.getOrNull(0)?.takeIf { it.isNotBlank() } ?: "玩家"
        val address = parts.getOrNull(1)?.takeIf { it.isNotBlank() } ?: "本地"
    ...
    private fun refreshHomeData() {
        ...
        val resumeSummary = if (resumeAvailable && saved != null) {
            ...
            val label = if (saved.mode == GameMode.DAILY) "每日一题" else saved.difficulty.label
        ...
        val recordSummary = if (record != null) {
            val label = if (record.mode == GameMode.DAILY) "每日一题" else record.difficulty.label
